...
(function (EPAL) {
  ...
  var GROUP_MODULES = [
    ...
    m('companies',      'Sister Concerns',      'diagram-3-fill',{ desc:'All companies at a glance — revenue, margin, risk.', sectionEnd:true }),
    ...
    m('crm',            'Group CRM',            'people-fill',   { desc:'Unified customer graph shared across all concerns.',
        subs:[['leads','Leads'],['pipeline','Pipeline'],['customers','Customers 360'],['activities','Activities']], sectionEnd:true }),
    ...
    m('meetings',       'Meetings & Notes',     'calendar2-week-fill',{ desc:'Call meetings with anyone in the group, notify them, and keep the agenda, minutes, decisions & actions in one thread.',
        subs:[['agenda','Agenda'],['calendar','Calendar'],['minutes','Minutes & Notes'],
    ...
    m('documents',      'Document Center',      'file-earmark-richtext-fill',{ desc:'Every branded invoice, receipt & report — searchable.', sectionEnd:true }),
  ...
  var TRAVELS_MODULES = [
    ...
    m('contract-flight', 'Contract Flight',  'airplane-engines', { desc:'Group/charter seat blocks (Umrah, Hajj, worker).',
        subs:[['schedule','Flight Schedule'],['day-board','Departures Board'],['add-flight','Add Flight'],['category','Category'],['manage-sales','Manage Sales']] }),
  ...
  var WOODART_MODULES = [
    ...
    /* The project BREAKDOWN, sitting next to `projects` because that is the
     * hierarchy it extends: project → space (Bed Room · Kitchen · Dining) →
     * phase (Design → Colour → Wood Work → Furniture), each phase owned by one
