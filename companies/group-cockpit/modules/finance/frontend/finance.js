...
var EPAL = window.EPAL = window.EPAL || {};

/* ---- template plumbing: real-HTML screens authored in template.html.
 * screen()/shell() clone a plain-HTML block; fillK writes [data-k] values;
 * data grids append into [data-fill]; mountScreen moves element children onto
 * the page so the final DOM equals the old el()-built output. ------------- */
var TPL = document.createElement('div');
TPL.innerHTML = TEMPLATE_HTML;
function frag(name) { var t = TPL.querySelector('template[data-tpl="' + name + '"]'); return t.content.firstElementChild.cloneNode(true); }
...
function mountScreen(page, s) { Array.prototype.slice.call(s.children).forEach(function (c) { page.appendChild(c); }); }
  ...
  function groupRedFlags() {
    ...
    if (dueSlips.length) {
      var amt = dueSlips.reduce(function (a, s) { return a + ((EPAL.payroll && EPAL.payroll.slipDue) ? EPAL.payroll.slipDue(s) : Math.max(0, (s.earnedGross - s.tax - s.pf) - (s.paid || 0))); }, 0);
