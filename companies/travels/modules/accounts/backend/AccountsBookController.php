...
use Epal\Modules\Travels\Accounts\Services\AccountsBookService;
use Illuminate\Http\JsonResponse;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Schema;

/**
 * Travels Accounts books API — serves the three module-owned stores
 * (recurring / cheques / petty) behind one controller keyed by {store}.
 * Schema::hasTable-guarded so it no-ops before migrate.
 */
class AccountsBookController
{
    private const TABLES = [
        'recurring' => 'tv_recurring',
        'cheques'   => 'tv_cheques',
        'petty'     => 'tv_petty',
    ];

    public function __construct(private AccountsBookService $service) {}
    ...
    {
        if (! $this->ready($store)) {
            return response()->json(['success' => false, 'message' => 'Travels accounts tables not migrated yet. Run: php artisan migrate'], 503);
        }
        $record = $request->all();
