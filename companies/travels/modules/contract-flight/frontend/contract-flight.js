 ...
 *
 * Block-seat inventory for the Travels desk. Epal buys blocks of seats up-front
 * on charters/allotments (Umrah, Hajj, worker & tourist runs) and re-sells them
 * seat-by-seat. Because the whole block is PRE-BOUGHT, every unsold seat is
 * trapped working capital and a hard deadline (departure day), so the module is
 * built around one anxiety: "sell the block before it flies".
 *
 * ONE registered view handles every sub-route (schedule / day-board / add-flight
 * / category / manage-sales). Data lives in store `tv_contract_flights`. Selling
 * seats fires db.postSale('travels',…) so Travels + Group finance + the ledger
 * all move live.
 *
 * ==> LARAVEL / PHP MAPPING: a ContractFlight model + FlightScheduleController
 *     (index/store), seat-sale posting through the shared sales/ledger service.
 * ========================================================================== */
...
function sectionNav(sub) {
  ...
  SECTIONS.forEach(function (s) {
    var b = frag('nav-btn');
    if (sub === s[0]) b.classList.add('active');
    b.textContent = s[1];
    b.addEventListener('click', function () { EPAL.router.navigate('travels/contract-flight' + (s[0] === 'schedule' ? '' : '/' + s[0])); });
    nav.appendChild(b);
  });
  return nav;
}
...
function schedule(page) {
  ...
  function draw() {
    ...
    var kgS = frag('kpi-grid-slim');
    kgS.appendChild(kpi('Total Seats', ui.num(totSeats), 'grid-3x3-gap-fill', function(){ flightsModal('Seat Blocks — '+ui.num(totSeats)+' seats', 'grid-3x3-gap-fill', [['Seats', ui.num(totSeats)], ['Sold', ui.num(totSold)], ['Unsold', ui.num(totUnsold)], ['Blocks', fl.length]], fl); }));
    kgS.appendChild(kpi('Seats Sold', ui.num(totSold), 'check2-circle', function(){ flightsModal('Seats Sold — '+ui.num(totSold), 'check2-circle', [['Sold', ui.num(totSold)], ['Load factor', lf+'%'], ['Revenue', ui.money(revenue)]], fl); }));
    kgS.appendChild(kpi('Load Factor', lf + '%', 'speedometer2', function(){ var byL=fl.slice().filter(function(f){return f.seats;}).sort(function(a,b){ return loadPct(b)-loadPct(a); }); flightsModal('Load Factor — '+lf+'%', 'speedometer2', [['Overall', lf+'%'], ['Best', byL[0]? byL[0].airline+' '+loadPct(byL[0])+'%':'—'], ['Weakest', byL.length? byL[byL.length-1].airline+' '+loadPct(byL[byL.length-1])+'%':'—']], fl); }));
    kgS.appendChild(kpi('Unsold Seats', ui.num(totUnsold), 'exclamation-diamond', function(){ flightsModal('Unsold Seats — '+ui.num(totUnsold), 'exclamation-diamond', [['Unsold', ui.num(totUnsold)], ['At-risk blocks', riskCount]], fl.filter(function(f){ return unsoldOf(f)>0; })); }));
    kgS.appendChild(kpi('Blocked Capital', ui.money(blocked, { compact:true }), 'lock-fill', function(){ flightsModal('Blocked Capital — '+ui.money(blocked), 'lock-fill', [['Trapped capital', ui.money(blocked)], ['Unsold seats', ui.num(totUnsold)]], fl.filter(function(f){ return unsoldOf(f)>0; })); }));
    kgS.appendChild(kpi('Revenue', ui.money(revenue, { compact:true }), 'cash-coin', function(){ flightsModal('Seat-Sale Revenue — '+ui.money(revenue), 'cash-coin', [['Revenue', ui.money(revenue)], ['Seats sold', ui.num(totSold)], ['Avg / seat', ui.money(totSold?Math.round(revenue/totSold):0)]], fl); }));
    kgS.appendChild(kpi('Net P&L', ui.money(netPnl, { compact:true }), netPnl>=0?'graph-up-arrow':'graph-down-arrow', function(){ pnlModal(fl); }));
    host.appendChild(kgS);
...
function category(page) {
  ...
  var kgC = frag('kpi-grid');
  kgC.appendChild(kpi('Categories Active', String(cats.length), 'diagram-3-fill'));
  kgC.appendChild(kpi('Seats Contracted', ui.num(totSeats), 'grid-3x3-gap-fill'));
  kgC.appendChild(kpi('Seats Unsold', ui.num(totSeats - totSold), 'exclamation-diamond'));
  kgC.appendChild(kpi('Revenue Booked', ui.money(totRev, { compact:true }), 'cash-stack'));
  page.appendChild(kgC);
...
function manageSales(page) {
  ...
  var kgM = frag('kpi-grid');
  kgM.appendChild(kpi('Seat-Sale Revenue', ui.money(revenue, { compact:true }), 'cash-coin'));
  kgM.appendChild(kpi('Seat Cost', ui.money(cost, { compact:true }), 'wallet2'));
  kgM.appendChild(kpi('Gross Profit', ui.money(revenue - cost, { compact:true }), 'graph-up-arrow'));
  kgM.appendChild(kpi('Seats Sold (all blocks)', ui.num(seatsSold), 'ticket-perforated-fill'));
  page.appendChild(kgM);
...
function dayBoard(page) {
  ...
  var kgD = frag('kpi-grid-compact');
  kgD.appendChild(kpi('Departed · ' + ui.date(yesterday), fl.filter(function (f) { return dep(f) === yesterday && f.status === 'Departed'; }).length + ' / ' + fl.filter(function (f) { return dep(f) === yesterday; }).length, 'check2-circle'));
  kgD.appendChild(kpi('Today', todayAll.length + ' flight' + (todayAll.length === 1 ? '' : 's'), 'calendar-day'));
  kgD.appendChild(kpi('Today — still to go', String(todayAll.filter(function (f) { return f.status !== 'Departed'; }).length), 'clock-history'));
  kgD.appendChild(kpi('Tomorrow', String(fl.filter(function (f) { return dep(f) === tomorrow; }).length), 'calendar-plus'));
  kgD.appendChild(kpi('Next 7 Days', String(fl.filter(function (f) { return dep(f) >= TODAY && dep(f) <= weekEnd(); }).length), 'calendar-week'));
  page.appendChild(kgD);
...
function kpi(label, value, icon, onClick) {
  var n = frag('kpi');
  if (onClick) { n.classList.add('drill'); n.addEventListener('click', onClick); }
