/* ============================================================================
 * TRAVELS · MARKETING & MESSAGING · LOGIC
 * ----------------------------------------------------------------------------
 * Behaviour only — the static markup lives in frontend/template.html and is
 * handed to this file (by tools/build/build-module.mjs) as the string
 * TEMPLATE_HTML. This file is NOT an IIFE and has no 'use strict' of its own:
 * the build wraps it in `(function(){ 'use strict'; var TEMPLATE_HTML=…; <this
 * file> })()`.
 *
 * Travels' omni-channel "live comms channel". ONE registered view
 * (travels/marketing) drives four workspaces from local pill state (ctx.subId is
 * honoured if the router supplies one):
 *   campaigns (default) → campaign blaster: KPIs + filterable table + CSV + New
 *                         Campaign + a "Send now" that counts recipients from
 *                         live stores, marks Sent, appends to the send-log.
 *   templates          → message-template library (CRUD) with a live {{name}} /
 *                         {{destination}} / {{fare}} placeholder preview.
 *   bot                → an interactive WhatsApp booking-bot simulator answering
 *                         from REAL data (fares / airlines / visa apps).
 *   sendlog            → delivery ledger of every tv_messages row.
 * All new stores self-seed idempotently via EPAL.registerEngine; persistence is
 * only ever through EPAL.store / EPAL.db.
 *
 * ==> LARAVEL / PHP MAPPING: Campaign / Template / Message models +
 *     CampaignController (index/store/send), a BotController for the assistant.
 * ========================================================================== */
  ...
  render: function (ctx) {
    ...
    function drawPills() {
      ...
      TABS.forEach(function (t) {
        var b = frag('nav-btn');
        if (state.tab === t[0]) b.classList.add('active');
        b.innerHTML = ui.icon(t[2]) + ' ' + t[1];
...
function renderCampaigns(host) {
  ...
  kg.appendChild(kpi('Campaigns', list.length, 'megaphone'));
  kg.appendChild(kpi('Messages Sent', ui.num ? ui.num(totalSent) : String(totalSent), 'send'));
  kg.appendChild(kpi('Delivery Rate', delivRate + '%', 'check2-circle'));
  kg.appendChild(kpi('Open Rate', openRate + '%', 'envelope-open'));
  host.appendChild(kg);

  var hr = frag('head-row');
  hr.appendChild(el('div.section-label', { text: 'Campaigns', style: { margin: '0' } }));
  hr.appendChild(el('button.btn.btn-primary', { html: ui.icon('plus-lg') + ' New Campaign',
...
function renderTemplates(host) {
  ...
  var hr = frag('head-row');
  hr.appendChild(el('div.section-label', { text: 'Message Templates', style: { margin: '0' } }));
  hr.appendChild(el('button.btn.btn-primary', { html: ui.icon('plus-lg') + ' New Template',
...
function renderSendLog(host) {
  ...
  var kg = frag('kpi-grid-plain');
  kg.appendChild(kpi('Sends Logged', list.length, 'send-check'));
  kg.appendChild(kpi('Total Delivered', num(totalDeliv), 'check2-all'));
  kg.appendChild(kpi('Total Opened', num(totalOpen), 'envelope-open'));
  kg.appendChild(kpi('Avg Open Rate', (totalDeliv ? Math.round(totalOpen / totalDeliv * 100) : 0) + '%', 'graph-up-arrow'));
  host.appendChild(kg);
