 ...
 *
 * The people desk for Epal Travels: the team directory, an attendance board, a
 * leave register, an engine-driven payroll run (posts salaries into the GL), and
 * a performance view. ONE registered view branches on ctx.subId (pill-tabs), and
 * — because the router prefers a specific view over the shared wildcard — this
 * Travels screen supersedes the generic one WITHOUT touching any other company.
 *
 * DATA: employees (shared HR store, db.employees({companyId:'travels'})),
 *   tv_leaves (this module's own store, seeded below), gl_entries (payroll runs).
 *
...
EPAL.view('travels/hrm', {
  render: function (ctx) {
    ...
    [['directory', 'Directory'], ['attendance', 'Attendance'], ['leaves', 'Leaves'], ['notices', 'Notices'], ['setup', 'HR Setup'], ['performance', 'Performance']].forEach(function (p) {
      var b = frag('nav-btn');
      if (sub === p[0]) b.classList.add('active');
      b.textContent = p[1];
      b.addEventListener('click', function () { EPAL.router.navigate('travels/hrm' + (p[0] === 'directory' ? '' : '/' + p[0])); });
      pills.appendChild(b);
    });
...
function directoryView(page) {
  ...
  kg.appendChild(kpi('Team Size', String(t.length), 'people'));
  kg.appendChild(kpi('Attendance', avgRate + '%', 'check2-circle', avgRate >= 90 ? 'text-good' : avgRate >= 80 ? 'text-warn' : 'text-bad'));
  kg.appendChild(kpi('Monthly Payroll', ui.money(payroll, { compact: true }), 'cash-stack'));
  kg.appendChild(kpi('Avg Rating', avgRating, 'star-fill'));
  kg.appendChild(kpiDrill('On Leave', String(onLeave), 'calendar2-x', 'travels/hrm/leaves', onLeave ? 'currently away' : 'all present'));
  page.appendChild(kg);
...
function attendanceView(page) {
  ...
  var avgRate = t.length ? Math.round(t.reduce(function (a, e) { return a + attRate(e); }, 0) / t.length) : 0;
  var kg = frag('kpi-grid');
  kg.appendChild(kpi('Avg Attendance', avgRate + '%', 'check2-circle', avgRate >= 90 ? 'text-good' : 'text-warn'));
  kg.appendChild(kpi('Present (Σ)', String(tot.present), 'person-check', 'text-good'));
  kg.appendChild(kpi('Absent (Σ)', String(tot.absent), 'person-x', tot.absent ? 'text-bad' : ''));
  kg.appendChild(kpi('Late (Σ)', String(tot.late), 'clock', tot.late ? 'text-warn' : ''));
  kg.appendChild(kpi('On Leave (Σ)', String(tot.leave), 'calendar2-x'));
  page.appendChild(kg);
...
function leavesView(page) {
  ...
  kg.appendChild(kpi('Approved', String(approved.length), 'check2-circle', 'text-good'));
  kg.appendChild(kpi('On Leave Now', String(onLeaveNow), 'calendar2-x'));
  kg.appendChild(kpi('Days Taken', String(totalDays), 'calendar-week'));
  kg.appendChild(kpi('Requests', String(all.length), 'card-list'));
  page.appendChild(kg);
...
function payrollView(page) {
  ...
  var kg = frag('kpi-grid');
  kg.appendChild(kpi('Headcount', String(slips.length), 'people'));
  kg.appendChild(kpi('Gross Payroll', ui.money(gross, { compact: true }), 'cash-stack'));
  kg.appendChild(kpi('Net Payable', ui.money(netPay, { compact: true }), 'wallet2'));
  kg.appendChild(kpi('Paid', ui.money(paid, { compact: true }), 'check2-circle', 'text-good'));
  kg.appendChild(kpi('Outstanding', ui.money(due, { compact: true }), 'hourglass-split', due > 0 ? 'text-warn' : 'text-good'));
  page.appendChild(kg);
...
function performanceView(page) {
  ...
  var kg = frag('kpi-grid');
  kg.appendChild(kpi('Avg Rating', avg.toFixed(1), 'star-fill'));
  kg.appendChild(kpi('Top Performer', top ? top.name.split(' ')[0] : '—', 'trophy'));
  kg.appendChild(kpi('Excellent', String(buckets['4.5–5 · Excellent']), 'award', 'text-good'));
  kg.appendChild(kpi('Needs Support', String(buckets['Below 3 · Needs support']), 'life-preserver', buckets['Below 3 · Needs support'] ? 'text-warn' : ''));
  page.appendChild(kg);
