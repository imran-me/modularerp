...
| **`scope`** (Spaces & Phases) ⭐ | `#/woodart/scope/{spaces,phases,load}` | ✅ real HTML, `[data-proto]` cards + rows, all utilities Tailwind (0 new classes), `api.js` seam owning the **space → phase hierarchy**, the derived progress/status and the assignment read of `employees` | ✅ 21-file slice — 4 migrations, 4 models, ScopeService, 4 controllers, requests, resources, routes, seeder; wired into api.js HYDRATE + CONDITIONAL | ✅ to the standard · migrated on the live host 2026-08-07 |
...
| Date | What | Commit |
|---|---|---|
| 2026-08-08 | **A DELETE COSTS THE SERVER ONE REQUEST, HOWEVER MANY ROWS IT CLEARS** — the fix for the failure the owner hit on the live site within the hour: deleting the project produced a screenful of *"Delete failed: Database rejected the write: Operation not permitted"* over an emptied register. Nothing about the delete logic was wrong; the **shape** was. The cascade ran in the browser as one `db.remove` per row, and every write on a WRITABLE store is its own HTTP call — so a project with 11 rooms, 86 phases and 214 planned lines fired **372 requests at once**. Shared hosting caps concurrent PHP workers and MySQL connections and answers the overflow with "Operation not permitted"; this repo already documents it twice (`api.js` hydration was throttled for the same reason, and `docs/PAYROLL-DEMO-DATA.md` records ~200 postings doing it). Now `ProjectController::destroy` runs the whole cascade in ONE transaction and the browser clears its own copy with a new `db.removeLocal` (the existing `local` flag api.js already skips) — **372 requests → 1**, proven by a probe that counts what would leave the browser. The same shape was fixed in the four other cascades: a room (server already cascaded — the client was duplicating work that could only fail), a phase, a drawing's revision trail, and a material's movement ledger; and `deleteOrder` gained the **order-line cascade it never had**, on both sides — an order line outliving its order still answered "how much is already on order", so a deleted order kept suppressing its items in Material Demand and they were never re-bought. Two more: `store()` now uses `withTrashed()->firstOrNew()` like `ClientService`, because deletes are soft and the browser numbers the next project from what it can see — delete WAP-101 and the next one is offered WAP-101 again, straight into the unique index; and a failed delete now raises **one toast per store**, not one per row. Cascade probe 6/6, delete probe 16/16, scope 34/34, books ✓, sweep 258/258 both themes. | — |
| 2026-08-08 | **DELETE, EVERYWHERE IT IS OWED** (owner: *"make a delete option everywhere"*, pointing at the project card). An audit driven from the DOM — not from grep — walked all 21 Interior screens and found **one** genuine gap: a project could be created but never removed. It now has a trash icon on its register card and a Delete in the profile header, and the delete **cascades the whole job**: rooms, phases, planned lines, estimates, budget heads, drawings, revisions, workshop jobs, site visits, purchase orders and order lines, each counted in the confirmation first. Two things it refuses to touch — **stock movements** and **book entries** — because the material really did leave the store and the money really did move; erasing either would silently change stock levels or unbalance the trial balance, so the sanctioned corrections stay an **Adjustment** and a **void/reversal**. The audit also caught a bug the whole app carried: **`ui.confirm` reads `opts.text`, and ~20 call sites across all three companies pass `opts.body`** — every one of those dialogs had been showing the fallback *"Are you sure?"* instead of the explanation its author wrote, my own delete confirms included. The helper now accepts either (and a node), restoring the written text at every site at once. And no destructive confirm in Interior set `danger: true`, so 13 Delete/Void buttons were blue; they are red now. Coverage after: **16/16 screens that own a record can delete it**; the four that cannot are read-across lenses (BOQ · Approvals · Snag List · Material Demand) which borrow rows another screen owns, plus the movement ledger, which is a ledger. Sweep **258/258 both themes**, scope 34/34, books story/refs/stock/trial ✓, tailwind gate ✓, and a probe driving the real button end to end 13/13. | — |
| 2026-08-07 | **QUANTITIES AND UNITS FIXED**, both reported from the live site. The purchase-order register showed the number of LINES in a column a reader takes for quantity, so a one-line order for 34,500 bricks read as **1**; it now shows what was ordered, from the order lines, with its unit (34,500 pcs · 10,075 kg · 502 bag). And no quantity anywhere in the movement ledger carried a unit — 1,000 of rod is kilos, of brick is pieces — so the unit now travels with every quantity in the stock ledger, the project ledger, the material history and the receive/issue form label, read from the material so it cannot contradict the register. A lump-sum contract genuinely has qty 1, which still read as if the system could not count, so work lines now say **lump sum** and **12 man-days** instead. |
| 2026-08-07 | **MATERIAL GETS A LEDGER, the way money already has one** (owner: *"a room needs 500 brick, so can buy 100, then 50 like that"* and *"like for money we have clear ledger etc history to see each transaction"*). Stock used to arrive in one lump and leave in another; it now arrives in **deliveries against a purchase order** and leaves **room by room** — 24 movements became 70, the same stock told as its history, with the invariant untouched (`books.mjs stock` still proves sum(movements) = stock). New store `wa_purchase_lines` — what an order actually orders, per material, because an order carrying only a total and an item count gave a part-delivery nothing to be part of. `wa_movements` gained `phase`, `space` and `order`. Rooms take their share by area using the same largest-remainder split as the requirements, so "this room needed 6,250 bricks and used 5,800" reconciles both ways. Shipped with its Laravel half in the same pass — migrations, model, resource, controller, routes, seeders, api.js wiring. |
| 2026-08-07 | **THE PROJECT PROFILE** — clicking a project in the register now opens the whole job on one page (#/woodart/projects/WAP-101) instead of a small drawer: header (contract · quoted · spent · billed · left), one plain-English line saying what needs attention today, quoted-vs-spent per cost head with red only where spent has passed quoted, rooms with weighted progress, materials quoted against used, the full material ledger, and a link to every desk that feeds it. **Read-only on the owner's instruction** — budgeting, phase building and materials all stay in their dedicated sections, so no number has two places to be edited. Nothing was removed: the old drawer keeps its tabs and billing action, reached from the profile header. Written in the legacy el() idiom of that file, which converts with the module at its scheduled rebuild. |
