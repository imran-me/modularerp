...
|------|--------|------|-------------|--------|
| 2026-07-28 | **woodart / accounts** (2) | owner: "I need these all in the Interiors Account" — full Travels tab set, 3 tabs to **11**: Overview · Income · Expenses · Payables · Project P&L · Payroll · Recurring · Banks · Manage Cash · Journals · Schedules. Payroll + Manage Cash MOUNT THE SHARED DESKS (EPAL.payrollDesk / EPAL.cashDesk already take a cid), so Woodart runs the same code Travels does rather than a copy. 7 of 9 needed NO new table; only wa_recurring is new — migration + controller + seeder + CONDITIONAL hydration | **sweep 253/253** (was 245), 0 errors · tailwind green · import gate 23 files/46 refs · **MySQL: 6 rows, 5 active, 9,39,200 commitment, 5 due vs the 2026-07-05 clock** | 41b8e76 |
| 2026-07-28 | **woodart / accounts** | module #8 full-stack: Income & Expense · Vendor Payables · Project P&L. Owns NO table — a desk over the SHARED acc_entries, postings routed to the KERNEL services (expenses via ExpensePostingService, income via SalePostingService + a register row in one transaction; ReceiptPostingService rejected because it settles an existing receivable). Variance column = BOQ budget − material issued from the stock ledger | PHP -l 6/6 · import gate 23/45 · tailwind gate green · **sweep 245/245 both themes, 0 errors** · build-fresh gate green | e0b1169 |
...
| 2026-07-27 | **woodart / design** | module #6, first built from ROOT-MAP: Drawing Register · Approvals · Design Load + revision-trail drawer · 10-file slice (2 controllers, read-only revisions, 1 shared service) · frozen `endpoints.md` · CONDITIONAL stores | PHP lint clean · tw gate green · **sweep 241/241 both themes** · **backend 41/41 vs MySQL** (incl. the phase gate's dangerous case) | (this) |
| 2026-07-27 | **woodart / installation** | module #5 full-stack: Schedule · Snag List · Teams · `api.js` seam owning the dual-shape snag count · 8-file slice recomputing the count on write · frozen `endpoints.md`. Handover billing NOT wired (projects already posts it) | PHP `-l` 8/8 · tw gate green · **sweep 237/237 both themes** · **backend 42/42 vs MySQL** | (this) |
